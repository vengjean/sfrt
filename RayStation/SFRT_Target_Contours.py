...
class MyWindow(Window):
    ...
    def update_list_of_geometries(self):
        ...
        # Set default to first item in list with name "GTV"
        if "GTV" in roi_names:
            lth.FindLogicalNode(
                self.window, "SelectGTVComboBox").SelectedItem = "GTV"
            self.gtv_name = "GTV"
        ...
        # Set default to first item in list with name "PTV_LD"
        if "Eval_PTV_LD" in roi_names:
            lth.FindLogicalNode(
                self.window, "SelectPTVComboBox").SelectedItem = "Eval_PTV_LD"
            self.ptv_name = "Eval_PTV_LD"
        elif "PTV_LD" in roi_names:
            lth.FindLogicalNode(
                self.window, "SelectPTVComboBox").SelectedItem = "PTV_LD"
            self.ptv_name = "PTV_LD"
...
class SFRT_Contour():
    def __init__(self, attrs):
        ...
        self.params = {
            ...
            "gtv_name": attrs.get("gtv_name", "GTV"),
            "ptv_name": attrs.get("ptv_name", "PTV_LD"),
            "sphere_radius": self.radius,
        }
        self.threshold = attrs.get("threshold", 0)

    def check_existing_roi(self, list_roi_to_check=None, list_geometry_to_check=None):
        if list_roi_to_check is None:
            list_roi_to_check = ["GTV_core", "x_PTV_core", "x_OARs_all", "Initial_hot_spheres", "Initial_cold_spheres", "x_external_shrink"]
        if list_geometry_to_check is None:
            list_geometry_to_check = ["x_OARs_PRV", "VTV_HD", "VTV_LD"]
    ...
    def generate_contours(self, sender, e):
        ...
        if not enough_spheres:
            self.check_existing_roi(list_roi_to_check=["GTV_core", "x_PTV_core", "x_OARs_all"], list_geometry_to_check=["x_OARs_PRV"])
            self.radius = 0.5
        ...
        self.check_existing_roi(list_roi_to_check=["GTV_core", "x_PTV_core", "x_OARs_all"], list_geometry_to_check=[])

        # Ensure downstream naming conventions are met (GTV and PTV_LD)
        self._ensure_named_copy(self.params["gtv_name"], "GTV", "Orange", "Gtv")
        self._ensure_named_copy(self.params["ptv_name"], "PTV_LD", "Red", "Ptv")

        return True

    def _ensure_named_copy(self, source_name, expected_name, color, roi_type):
        """If source_name differs from expected_name, create a copy with the expected name."""
        if source_name == expected_name:
            return
        roi_list = [f.Name for f in self.case.PatientModel.RegionsOfInterest]
        if expected_name in roi_list:
            if self.structure_set.RoiGeometries[expected_name].HasContours():
                return
        roi = self._create_roi(expected_name, color, roi_type)
        roi.CreateMarginGeometry(
            Examination=self.exam,
            SourceRoiName=source_name,
    ...
    def _delete_geometry(self, roi_name):
        ...
        self.case.PatientModel.StructureSets[self.exam.Name].RoiGeometries[roi_name].DeleteRoiGeometry()
    ...
    def create_substructures(self, params):
        ...
        # Shrink the GTV_core by gtv_margin
        gtv_core = self._contract_roi(
            gtv_name, gtv_margin, "GTV_core", "Maroon", "Gtv")

        # Perform subtraction of GTV from the combined PRV
        gtv_core = self._subtract_roi(
            "GTV_core", combined_prv.Name, "GTV_core", "Maroon", "Gtv")
        # Shrink the GTV_core by sphere_radius
        gtv_core = self._contract_roi(
            "GTV_core", sphere_radius, "GTV_core", "Maroon", "Gtv")
        self._simplify_roi(gtv_core.Name)

        # Perform subtraction of PTV from the combined OAR
        ptv_core = self._subtract_roi(
            ptv_name, oar_combined.Name, "x_PTV_core", "Red", "Ptv")
        # Perform intersection of PTV with external_shrink
        ptv_core = self._intersection_roi(
            "x_PTV_core", external_shrink.Name, ptv_core.Name, "Red", "Ptv")
    ...
    def make_ptv_spheres(self, gtv_name, ptv_name):
        ...
        # Intersection of hot PTV with original GTV
        ...
        # Intersection of cold PTV with PTV_core
